...
void pinMode( uint32_t ulPin, uint32_t ulMode)
{
	...
	//add for dashbutten
    if(	g_APinDescription[ulPin].pinname==PE_0 ||
      g_APinDescription[ulPin].pinname==PE_1 ||
      g_APinDescription[ulPin].pinname==PE_2 ||
      g_APinDescription[ulPin].pinname==PE_3 ||
      g_APinDescription[ulPin].pinname==PE_4	)
    {
    	/* if A2 < 800, then do not disable jtag */
    	int battery=0, i=0;
		for (i = 0; i < 10; i++) {
			battery = analogRead(A2);
			delay(10);
		}
		if (battery < 800) { //low battery level 
		} else {
      		sys_jtag_off();
		}
    }
